...
public class FieldPersonnelActivity extends BaseActivity implements OnClickListener{
	private RelativeLayout rel_passed_status,rel_query_signin;
	private TextView tv_wq_status,tv_wq_name,tv_reason_time,tv_user_reason,tv_cq_time,tv_cq_address,tv_have_passedxq,tv_refusexq,tv_query_signin,tv_signin;
	private String applyId;
	private HttpManager http;
	private List<ApplyDetailed> applyDetailed;
	@Override
	...
	protected void initData() {
		// TODO Auto-generated method stub
		applyDetailed=new ArrayList<ApplyDetailed>();
	...
	private void getInitData(ApplyDetailed applyDetailed){
		if(applyDetailed.getApp_state().equals("1")){
			...
			rel_passed_status.setVisibility(View.VISIBLE);
	...
	HttpCallBackListener listener = new HttpCallBackListener(){
		...
		public void onSuccess(HttpMessage result, int tag, Object obj) {
			...
			switch (tag) {
				case 1:
					applyDetailed=JSON.parseArray(result.getData(), ApplyDetailed.class);
					getInitData(applyDetailed.get(0));
					break;

				case 2:
				case 3:
					Toast.makeText(FieldPersonnelActivity.this, result.getMsg(), Toast.LENGTH_SHORT).show();
					finish();
					break;
			}
		}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
			case R.id.tv_have_passedxq:
				http.doGet(ConstantsUrl.makeApplication("2",applyId), 2, false);
				break;

			case R.id.tv_refusexq:
				http.doGet(ConstantsUrl.makeApplication("3",applyId), 3, false);
				break;
			case R.id.tv_query_signin:
				if(applyDetailed.get(0).getIsTrack()==0){
					showToast("没有签到信息!");
				}else{
					Intent intent = new Intent(FieldPersonnelActivity.this,SignInInfoActivity.class);
					intent.putExtra("id", applyDetailed.get(0).getId());
