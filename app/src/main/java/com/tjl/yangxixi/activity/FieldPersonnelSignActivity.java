...
public class FieldPersonnelSignActivity extends BaseActivity implements OnClickListener{
	...
	//public static FieldPersonnelSignActivity instance = null;
	@Override
	...
	HttpCallBackListener listener = new HttpCallBackListener(){
		...
		public void onSuccess(HttpMessage result, int tag, Object obj) {
			...
			switch (tag) {
				...
				case 2:
				case 3:
					Toast.makeText(FieldPersonnelSignActivity.this, result.getMsg(), Toast.LENGTH_SHORT).show();
					finish();
					break;
			}
		}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
			case R.id.tv_have_passedxq:
				http.doGet(ConstantsUrl.makeApplication("3",applyId), 2, false);
				break;

			case R.id.tv_refusexq:
				http.doGet(ConstantsUrl.makeApplication("2",applyId), 3, false);
				break;
			case R.id.tv_query_signin:
				if(applyList.get(0).getIsTrack()==0){
					showToast("没有签到信息!");
				}else{
					Intent intent = new Intent(FieldPersonnelSignActivity.this,SignInInfoActivity.class);
					intent.putExtra("id", applyList.get(0).getId());
					startActivity(intent);
				}
				break;
			case R.id.tv_signin:
				Intent intent1 = new Intent(FieldPersonnelSignActivity.this,SignInMapActivity.class);
				intent1.putExtra("id", applyList.get(0).getId());
